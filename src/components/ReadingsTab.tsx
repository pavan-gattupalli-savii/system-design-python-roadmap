...
// Lazy: only the row renderer for the current viewport ever ships.
const ReadingsTableRow   = lazy(() => import("./ReadingsTableRow"));
const ReadingsMobileCard = lazy(() => import("./ReadingsMobileCard"));
        ...
        {loading ? (
          ...
          <Suspense fallback={<div style={{ padding: 40, textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>Loading…</div>}>
            <div style={{ display: "flex", flexDirection: "column" }}>
              {pagedReadings.map((r) => <ReadingsMobileCard key={r.id} r={r} myVotes={myVotes} toggleVote={toggleVote} isBookmarked={myBookmarks.has(r.id)} toggleBookmark={(id) => toggleBookmark("reading", id, "/app/readings")} />)}
            </div>
          </Suspense>
        ) : (
          <div style={{ overflowX: "auto" }}>
            <table style={{
              ...
              <tbody>
                <Suspense fallback={<tr><td colSpan={8} style={{ padding: 40, textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>Loading…</td></tr>}>
                  {pagedReadings.map((r, idx) => (
                    <ReadingsTableRow key={r.id} r={r} idx={idx} myVotes={myVotes} toggleVote={toggleVote} isBookmarked={myBookmarks.has(r.id)} toggleBookmark={(id) => toggleBookmark("reading", id, "/app/readings")} />
                  ))}
                </Suspense>
              </tbody>
            </table>

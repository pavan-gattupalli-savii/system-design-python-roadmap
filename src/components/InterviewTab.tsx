...
import { COMPANIES, EXP_PLATFORMS, EXP_OUTCOMES } from "../data/interviews";
import type { InterviewExp, ExpPlatform, ExpOutcome } from "../data/interviews";
import { apiFetch } from "../api/client";
import { buildExperiencesUrl } from "../api/interviews";
import { useAuth } from "../lib/auth";
import { useMyInteractions } from "../hooks/useMyInteractions";
import { useBookmarks } from "../hooks/useBookmarks";
import { qk } from "../lib/queryKeys";

// Lazy: Q&A section (~20KB) only mounts when user clicks the Q&A sub-tab.
const InterviewQASection = lazy(() => import("./InterviewQASection"));
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {section === "experiences" ? (
        ...
        <Suspense fallback={<div style={{ padding: 56, textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>Loading questions…</div>}>
          <InterviewQASection isMobile={isMobile} />
        </Suspense>
      )}
    </div>
  );

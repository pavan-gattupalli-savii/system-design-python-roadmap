...
export default function App() {
  ...
  function handleJumpToWeek(ph, wn) {
    setSelPhase(ph);
    setSelWeek(wn);
    setOpenSessions({ 0: true, 1: true, 2: true });
    setSearchQuery("");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", background: "#060a10", color: "#c9d8e8", fontFamily: FONT_STACK, overflow: "hidden" }}>
      ...
      <header style={{ background: "#0d1117", borderBottom: "1px solid #1c2430", padding: isMobile ? "10px 14px 0" : "12px 24px 0", flexShrink: 0 }}>
        ...
        {/* Tabs — rendered from TABS constant in constants/app.js */}
      ...
      <main style={{ flex: 1, overflow: "hidden", display: "flex" }}>
        {activeTab === "tracker" ? (
          ...
          <>
            {mobileView === "phases" && <PhasesPanel selPhase={selPhase} isMobile={true}  selectPhase={selectPhase} completed={completed} />}
            {mobileView === "weeks"  && <WeeksPanel  phase={phase}    selWeek={selWeek}  isMobile={true}  selectWeek={selectWeek} setMobileView={setMobileView} completed={completed} />}

    ...
    weeks: [
      { n: 5, title: "SOLID Principles", sessions: [
        ...
        { label: "Build", focus: "Refactor a messy class to SOLID", resources: [
          { type: "Build",   item: "Take a real messy class (e.g. God-object order handler) and refactor to SOLID", where: "Before/after comparison. Document every violation fixed.", mins: 60 },
      ...
      { n: 10, title: "Food Delivery + Design Docs", sessions: [
        ...
        { label: "Study", focus: "How to write a design doc", resources: [
          ...
          { type: "Article", item: "Google Design Doc format",                                         where: "Search 'google design doc template one-pager' — Problem → Goals → Design → Trade-offs", mins: 20 },
        ]},
...
const allWeeks = roadmap.flatMap(p => p.weeks.map(w => ({ ...w, phase: p.phase, accent: p.accent, light: p.light })));
...
// ── PHASES PANEL ──────────────────────────────────────────
function PhasesPanel({ roadmap, selPhase, isMobile, selectPhase }) {
  return (
    <div style={{ background: "#0d1117", overflowY: "auto", flex: isMobile ? 1 : "none", width: isMobile ? "100%" : 200, flexShrink: 0, borderRight: isMobile ? "none" : "1px solid #1c2430" }}>
      {roadmap.map(p => (
        <button key={p.phase} onClick={() => selectPhase(p.phase)} style={{
          width: "100%", textAlign: "left", background: selPhase === p.phase ? `${p.accent}18` : "transparent",
          borderLeft: isMobile ? "none" : selPhase === p.phase ? `3px solid ${p.accent}` : "3px solid transparent",
          borderRight: "none", borderTop: "none",
          borderBottom: "1px solid #111827",
          padding: isMobile ? "14px 16px" : "10px 16px",
          cursor: "pointer", display: "flex", flexDirection: isMobile ? "row" : "column",
          alignItems: isMobile ? "center" : "flex-start", gap: isMobile ? 14 : 3,
        }}>
          <span style={{ fontSize: isMobile ? 24 : 20, flexShrink: 0 }}>{p.icon}</span>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: selPhase === p.phase ? p.light : "#64748b" }}>Phase {p.phase}</div>
            <div style={{ fontSize: isMobile ? 13 : 11, color: selPhase === p.phase ? "#e2e8f0" : "#4b5563", lineHeight: 1.35, marginTop: 2 }}>{p.title}</div>
          </div>
...
// ── WEEKS PANEL ───────────────────────────────────────────
function WeeksPanel({ phase, selWeek, isMobile, selectWeek, setMobileView }) {
  return (
    <div style={{ background: "#090e16", overflowY: "auto", flex: isMobile ? 1 : "none", width: isMobile ? "100%" : 170, flexShrink: 0, borderRight: isMobile ? "none" : "1px solid #1c2430" }}>
      {isMobile && (
        <button onClick={() => setMobileView("phases")} style={{ width: "100%", background: "#0d1117", border: "none", borderBottom: "1px solid #1c2430", padding: "11px 16px", color: "#64748b", fontSize: 12, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 8 }}>
          ‹ All Phases
        </button>
      )}
      ...
      {phase?.weeks.map(w => (
        <button key={w.n} onClick={() => selectWeek(w.n)} style={{
          width: "100%", textAlign: "left",
          background: selWeek === w.n ? `${phase.accent}18` : "transparent",
          borderLeft: !isMobile && selWeek === w.n ? `2px solid ${phase.accent}` : "2px solid transparent",
          borderRight: "none", borderTop: "none",
          borderBottom: "1px solid #0d1117",
          padding: isMobile ? "13px 16px" : "9px 14px",
          cursor: "pointer", display: "flex", alignItems: isMobile ? "center" : "flex-start",
          flexDirection: isMobile ? "row" : "column", gap: isMobile ? 12 : 2,
        }}>
          <span style={{ fontSize: 11, color: selWeek === w.n ? phase.light : "#4b5563", letterSpacing: 1, textTransform: "uppercase", flexShrink: 0 }}>Wk {w.n}</span>
          <span style={{ fontSize: isMobile ? 13 : 11, color: selWeek === w.n ? "#e2e8f0" : "#6b7280", lineHeight: 1.4, flex: 1 }}>{w.title}</span>
          {isMobile && <span style={{ color: "#334155", fontSize: 16, flexShrink: 0 }}>›</span>}
        </button>
      ))}
    </div>
...
// ── DETAIL PANEL ──────────────────────────────────────────
function DetailPanel({ weekObj, phase, openSessions, toggleSession, isMobile, setMobileView, typeSummary, totalMins }) {
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: isMobile ? "14px 14px 80px" : "24px 28px" }}>
      {isMobile && (
        <button onClick={() => setMobileView("weeks")} style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 6, padding: "7px 14px", color: "#94a3b8", fontSize: 12, cursor: "pointer", marginBottom: 16, display: "flex", alignItems: "center", gap: 6 }}>
          ‹ {phase?.icon} Phase {weekObj?.phase} weeks
        </button>
      )}
      {weekObj ? (
        <>
          <div style={{ marginBottom: 18 }}>
            <div style={{ fontSize: 11, letterSpacing: 3, color: phase?.accent, textTransform: "uppercase", marginBottom: 5 }}>
              Phase {weekObj.phase} · Week {weekObj.n}
            </div>
            <h2 style={{ margin: 0, fontSize: isMobile ? 17 : 21, color: "#f0f6ff", fontWeight: 700, lineHeight: 1.3 }}>{weekObj.title}</h2>
            <div style={{ marginTop: 10, display: "flex", gap: 7, flexWrap: "wrap", alignItems: "center" }}>
              {typeSummary.map(({ type, count }) => {
                const tc = TYPES[type] || TYPES["Article"];
                return <span key={type} style={{ fontSize: 10, background: tc.bg, color: tc.tx, border: `1px solid ${tc.tx}30`, borderRadius: 4, padding: "2px 7px" }}>{tc.icon} {count}× {type}</span>;
              })}
              <span style={{ fontSize: 11, color: "#475569", marginLeft: "auto" }}>≈ {totalMins} min</span>
            </div>
          </div>
          ...
          {weekObj.sessions.map((session, si) => {
            ...
            return (
              <div key={si} style={{ marginBottom: 10, border: `1px solid ${isOpen ? "#21262d" : "#161b22"}`, borderRadius: 10, overflow: "hidden" }}>
                <button onClick={() => toggleSession(si)} style={{
                  width: "100%", textAlign: "left", padding: isMobile ? "11px 13px" : "13px 18px",
                  background: isOpen ? "#111827" : "#0d1117", border: "none", cursor: "pointer",
                  display: "flex", alignItems: "center", gap: 9, flexWrap: "wrap",
                }}>
                  <span style={{ fontSize: 10, letterSpacing: 2, color: lc.color, background: lc.bg, border: `1px solid ${lc.border}`, borderRadius: 4, padding: "2px 7px", flexShrink: 0, textTransform: "uppercase" }}>
                    {session.label}
                  </span>
                  <span style={{ fontSize: isMobile ? 12 : 14, fontWeight: 600, color: "#e2e8f0", flex: 1, minWidth: 0 }}>{session.focus}</span>
                  <span style={{ fontSize: 11, color: "#475569", flexShrink: 0 }}>{smins}m {isOpen ? "▲" : "▼"}</span>
                </button>
                {isOpen && (
                  <div style={{ padding: isMobile ? "6px 10px 12px" : "4px 18px 16px", background: "#090e16", display: "flex", flexDirection: "column", gap: 8 }}>
                    {session.resources.map((res, ri) => {
                      ...
                      return (
                        <div key={ri} style={{ background: tc.bg, border: `1px solid ${tc.tx}25`, borderRadius: 8, padding: isMobile ? "10px 12px" : "12px 16px" }}>
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                            <span style={{ fontSize: 10, color: tc.tx, background: `${tc.tx}15`, border: `1px solid ${tc.tx}30`, borderRadius: 4, padding: "2px 7px", letterSpacing: 1, textTransform: "uppercase" }}>
                              {tc.icon} {res.type}
                            </span>
                            <span style={{ fontSize: 11, color: "#475569" }}>{res.mins} min</span>
                          </div>
                          <div style={{ fontSize: isMobile ? 12 : 13, color: "#f0f6ff", lineHeight: 1.5, fontWeight: 500 }}>{res.item}</div>
                          <div style={{ fontSize: 11, color: "#64748b", marginTop: 5, lineHeight: 1.4 }}>
                            <span style={{ color: "#374151" }}>→ </span>{res.where}
                          </div>
                        </div>
                      );
                    })}
          ...
          <div style={{ marginTop: 14, background: "#0c1a0c", border: "1px solid #1a3a1a", borderRadius: 8, padding: "11px 13px" }}>
            <span style={{ fontSize: 11, color: "#4ade80", marginRight: 6 }}>💡 Tip:</span>
            <span style={{ fontSize: 12, color: "#6b7280", lineHeight: 1.5 }}>
              {weekObj.sessions[0]?.label === "Practice"
                ? "Mock interviews are your most valuable sessions. 30 min of deliberate practice beats 2 hours of passive reading."
                : "1-hour day? Do one Study. 2-hour day? Study + Build. Zero-day? No guilt — pick up where you left off."}
            </span>
          </div>
        </>
      ) : (
        <div style={{ color: "#374151", fontSize: 14 }}>Select a week to see resources.</div>
      )}
    </div>
  );
}
...
// ── TRACKER TAB ───────────────────────────────────────────
function TrackerTab({ roadmap }) {
  return (
    <div style={{ padding: "24px 28px", overflowY: "auto", flex: 1 }}>
      <h2 style={{ color: "#f0f6ff", fontSize: 20, fontWeight: 700, marginBottom: 6 }}>9-Month Progress Overview</h2>
      <p style={{ color: "#64748b", fontSize: 12, marginBottom: 24 }}>7 phases · 40 weeks · 280+ sessions — Working Professional Pace</p>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 14 }}>
        {roadmap.map(p => (
          <div key={p.phase} style={{ background: "#0d1117", border: `1px solid ${p.accent}30`, borderRadius: 12, padding: "16px 18px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              <span style={{ fontSize: 22 }}>{p.icon}</span>
              <div>
                <div style={{ fontSize: 10, color: p.accent, letterSpacing: 2, textTransform: "uppercase", marginBottom: 2 }}>Phase {p.phase}</div>
                <div style={{ fontSize: 13, color: "#e2e8f0", fontWeight: 600 }}>{p.title}</div>
              </div>
            </div>
            <div style={{ fontSize: 11, color: "#64748b", marginBottom: 10, lineHeight: 1.5 }}>{p.desc}</div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              <span style={{ fontSize: 10, background: "#161b22", color: "#94a3b8", borderRadius: 4, padding: "2px 8px" }}>
                {p.weeks.length} weeks
              </span>
              <span style={{ fontSize: 10, background: "#161b22", color: "#94a3b8", borderRadius: 4, padding: "2px 8px" }}>
                {p.weeks.reduce((a, w) => a + w.sessions.length, 0)} sessions
              </span>
              <span style={{ fontSize: 10, background: "#161b22", color: p.light, borderRadius: 4, padding: "2px 8px" }}>
                Wk {p.weeks[0].n}–{p.weeks[p.weeks.length - 1].n}
              </span>
            </div>
          </div>
        ))}
      ...
      <div style={{ marginTop: 32 }}>
        <h3 style={{ color: "#f0f6ff", fontSize: 16, fontWeight: 600, marginBottom: 16 }}>📚 Key Resources</h3>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 10 }}>
          {[
            { name: "Designing Data-Intensive Applications", author: "Martin Kleppmann", priority: "🔴 BIBLE", note: "Read alongside every phase" },
            { name: "System Design Interview Vol 1", author: "Alex Xu", priority: "🔴 MUST", note: "Best HLD interview prep" },
            { name: "System Design Interview Vol 2", author: "Alex Xu", priority: "🟡 High", note: "Advanced systems" },
            { name: "Fluent Python 2e", author: "Luciano Ramalho", priority: "🔴 MUST", note: "Core Phase 1 resource" },
            { name: "Head First Design Patterns", author: "Freeman & Robson", priority: "🟡 High", note: "Best visual intro to patterns" },
            { name: "Google SRE Book", author: "Beyer et al.", priority: "🟡 High", note: "FREE at sre.google — Phase 6" },
            { name: "ByteByteGo", author: "Alex Xu (YouTube)", priority: "🔴 MUST", note: "Best HLD visual explanations" },
            { name: "Hussein Nasser", author: "YouTube Channel", priority: "🔴 MUST", note: "Networking & backend deep dives" },
            { name: "ArjanCodes", author: "YouTube Channel", priority: "🔴 MUST", note: "Python patterns & best practices" },
            { name: "refactoring.guru", author: "Website", priority: "🟡 High", note: "SOLID + all design patterns" },
          ].map((r, i) => (
            <div key={i} style={{ background: "#0d1117", border: "1px solid #1c2430", borderRadius: 8, padding: "12px 14px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                <span style={{ fontSize: 11, color: "#94a3b8", fontWeight: 600 }}>{r.name}</span>
                <span style={{ fontSize: 10 }}>{r.priority}</span>
              </div>
              <div style={{ fontSize: 10, color: "#475569" }}>{r.author}</div>
              <div style={{ fontSize: 10, color: "#64748b", marginTop: 4, fontStyle: "italic" }}>{r.note}</div>
            </div>
          ))}
        </div>
      </div>
...
export default function App() {
  ...
  const [activeTab, setActiveTab]     = useState("roadmap"); // "roadmap" | "tracker"
  const [selPhase, setSelPhase]       = useState(1);
  const [selWeek,  setSelWeek]        = useState(1);
  const [openSessions, setOpenSessions] = useState({ 0: true, 1: true, 2: true });
  const [mobileView, setMobileView]   = useState("phases"); // "phases" | "weeks" | "detail"

  const phase   = roadmap.find(p => p.phase === selPhase);
  const weekObj = allWeeks.find(w => w.phase === selPhase && w.n === selWeek);

  const selectPhase = (ph) => {
    setSelPhase(ph);
    setSelWeek(roadmap.find(p => p.phase === ph).weeks[0].n);
  ...
  const toggleSession = (idx) => setOpenSessions(prev => ({ ...prev, [idx]: !prev[idx] }));

  const typeSummary = weekObj
    ? weekObj.sessions.flatMap(s => s.resources).reduce((acc, r) => {
        const f = acc.find(a => a.type === r.type);
        if (f) f.count++; else acc.push({ type: r.type, count: 1 });
        return acc;
      }, [])
    : [];
  const totalMins = weekObj?.sessions.flatMap(s => s.resources).reduce((a, r) => a + r.mins, 0) ?? 0;

  return (
    <div style={{ fontFamily: "'Courier New', monospace", background: "#080c12", minHeight: "100vh", color: "#c9d1d9", display: "flex", flexDirection: "column", maxWidth: "100vw", overflowX: "hidden" }}>

      {/* TOP BAR */}
      <div style={{ background: "#0d1117", borderBottom: "1px solid #1c2430", padding: isMobile ? "12px 14px" : "15px 28px", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 10, letterSpacing: 3, color: "#6366f1", textTransform: "uppercase", marginBottom: 3 }}>System Design Mastery</div>
            <div style={{ fontSize: isMobile ? 15 : 19, fontWeight: 700, color: "#f0f6ff" }}>
              9-Month Roadmap {!isMobile && <span style={{ fontSize: 12, color: "#64748b", fontWeight: 400 }}>— Working Professional Pace</span>}
            </div>
          </div>
          <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
            {(isMobile
              ? [["40", "Wks"], ["7", "Ph"], ["280+", "Sess"]]
              : [["40", "Weeks"], ["7", "Phases"], ["280+", "Sessions"], ["1", "Capstone"]]
            ).map(([v, l]) => (
              <div key={l} style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 7, padding: isMobile ? "4px 9px" : "6px 12px", textAlign: "center" }}>
                <div style={{ fontSize: isMobile ? 13 : 16, fontWeight: 700, color: "#f1f5f9" }}>{v}</div>
        ...
        <div style={{ display: "flex", gap: 4, marginTop: 12 }}>
          {[["roadmap", "🗺️ Roadmap"], ["tracker", "📋 Overview & Resources"]].map(([tab, label]) => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{
              ...
              color: activeTab === tab ? "#f0f6ff" : "#64748b",
              fontFamily: "inherit",
            }}>{label}</button>
          ))}
        </div>
      ...
      {/* PACE BAR */}
      <div style={{ background: "#0c1a2e", borderBottom: "1px solid #1e3a5f", padding: isMobile ? "7px 14px" : "7px 28px", display: "flex", gap: isMobile ? 10 : 24, alignItems: "center", flexWrap: "wrap", flexShrink: 0, overflowX: "auto" }}>
        <span style={{ fontSize: 10, color: "#3b82f6", letterSpacing: 2, textTransform: "uppercase", flexShrink: 0 }}>Pace:</span>
        {(isMobile
          ? [["🐢 Relaxed", "1yr+"], ["⚖️ Balanced", "9–10mo"], ["🚀 Focused", "6mo"]]
          : [["🐢 Relaxed", "1yr+", "5–7 sessions/wk"], ["⚖️ Balanced", "9–10mo", "4–5 sessions/wk"], ["🚀 Focused", "6mo", "7 sessions/wk"]]
        ).map((row) => (
          <span key={row[0]} style={{ fontSize: 11, color: "#94a3b8", flexShrink: 0 }}>
            <span style={{ color: "#e2e8f0", fontWeight: 600 }}>{row[0]}</span> {row[1]}{row[2] ? ` — ${row[2]}` : ""}
          </span>
        ))}
      </div>
      ...
      {/* BODY */}
      {activeTab === "tracker" ? (
        <TrackerTab roadmap={roadmap} />
      ) : isMobile ? (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minHeight: 0 }}>
          <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
            {mobileView === "phases" && <PhasesPanel roadmap={roadmap} selPhase={selPhase} isMobile={isMobile} selectPhase={selectPhase} />}
            {mobileView === "weeks"  && <WeeksPanel  phase={phase} selWeek={selWeek} isMobile={isMobile} selectWeek={selectWeek} setMobileView={setMobileView} />}
            {mobileView === "detail" && <DetailPanel weekObj={weekObj} phase={phase} openSessions={openSessions} toggleSession={toggleSession} isMobile={isMobile} setMobileView={setMobileView} typeSummary={typeSummary} totalMins={totalMins} />}
          </div>
          {/* BOTTOM NAV */}
          <div style={{ background: "#0d1117", borderTop: "1px solid #1c2430", padding: "8px 12px", display: "flex", gap: 6, flexShrink: 0 }}>
            {[["phases", "Phases", "📋"], ["weeks", "Weeks", "📅"], ["detail", "Session", "📖"]].map(([v, l, icon]) => (
              <button key={v} onClick={() => {
                if (v === "phases") setMobileView("phases");
                else if (v === "weeks" && mobileView !== "phases") setMobileView("weeks");
              }} style={{
                flex: 1, padding: "8px 4px", borderRadius: 8, border: "none", cursor: "pointer",
                background: mobileView === v ? `${phase?.accent}20` : "#111827",
                color: mobileView === v ? phase?.light : "#475569",
        ...
        <div style={{ display: "flex", flex: 1, overflow: "hidden", minHeight: 0 }}>
          <PhasesPanel roadmap={roadmap} selPhase={selPhase} isMobile={isMobile} selectPhase={selectPhase} />
          <WeeksPanel  phase={phase} selWeek={selWeek} isMobile={isMobile} selectWeek={selectWeek} setMobileView={setMobileView} />
          <DetailPanel weekObj={weekObj} phase={phase} openSessions={openSessions} toggleSession={toggleSession} isMobile={isMobile} setMobileView={setMobileView} typeSummary={typeSummary} totalMins={totalMins} />
        </div>
      )}
    </div>
  );
